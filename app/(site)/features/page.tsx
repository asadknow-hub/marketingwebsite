    ...
    <>
      ...
      >
        <div className="relative w-full max-w-xl">
          ...
          <input
            ...
            className="w-full h-13 py-3.5 pl-12 pr-4 rounded-full border border-[#15122E]/10 bg-white/80 font-['DM_Sans'] text-[#15122E] placeholder:text-[#15122E]/40 outline-none focus:border-[#6C63FF] focus:ring-2 focus:ring-[#6C63FF]/20 transition-all"
          />
        </div>
      </PageHero>

      {/* Stats */}
      <section className="w-full px-4 sm:px-6 lg:px-8 pb-12 bg-[#F6F7FB]">
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 pb-24 bg-[#F6F7FB]">
        <div className="max-w-[1100px] mx-auto space-y-4">
          {filtered.length === 0 && (
            <p className="text-center py-16 font-['DM_Sans'] text-[#15122E]/40">
          ...
          {filtered.map((mod) => {
            ...
            return (
              <div
                key={mod.id}
                className="rounded-[28px] border border-[#15122E]/10 bg-white overflow-hidden"
              >
                <button
                  onClick={() => toggle(mod.id)}
                  className="w-full flex items-center justify-between gap-4 p-6 text-left hover:bg-[#15122E]/[0.03] transition-colors"
                >
                    ...
                    >
                      <div className="px-6 pb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {visibleFeatures.map((f) => (
                          <div
                            key={f.name}
                            className="p-4 rounded-2xl border border-[#15122E]/8 hover:border-[#6C63FF]/40 hover:bg-[#6C63FF]/[0.06] transition-all"
                          >
                            <div className="flex items-start gap-3">
                              <div className="w-5 h-5 rounded-full bg-[#6C63FF]/15 flex items-center justify-center shrink-0 mt-0.5">
                                <Check className="w-3.5 h-3.5 text-[#6C63FF]" strokeWidth={3} />
                              </div>
                              <div>
                                <div className="font-onest text-[15px] font-semibold text-[#15122E]">
                                  {f.name}
                                </div>
                                <div className="font-['DM_Sans'] text-[13px] text-[#15122E]/50 leading-relaxed">
                                  {f.description}
                                </div>
                              </div>
                            </div>
